...
public class App {
    public static void main(String[] args) {
        System.out.println(howManyLightsAreLit(instructions()).toString());
        // Result[lightsLit=377891, brightness=14110788]
    }

    public static Result howManyLightsAreLit(String instructions) {
        var grid = new LightningGrid();
        Arrays.stream(instructions.split("\n"))
                .filter(s -> !s.isEmpty())
                .map(String::trim)
                .map(Instruction::new)
                .forEach(grid::applyInstrunction);
        return new Result(grid.howManyLightsAreLit(), grid.totalBrightness());
    }

    record Result(long lightsLit, long brightness) {}
    static class LightningGrid{
        private Light[][] grid = new Light[1000][1000];

        public LightningGrid() {
            for (Light[] row : grid) {
        ...
        void applyInstrunction(Instruction instruction){
                var rec = instruction.getRectangle();

                for (int y = rec.corner.y(); y <= rec.otherCorner.y(); y++) {
                    for (int x = rec.corner.x(); x <= rec.otherCorner.x(); x++) {
                        instruction.applyTo(grid[y][x]);
                    }
                }
        }

        long howManyLightsAreLit() {
    ...
    record Instruction(String value){
        void applyTo(Light light) {
            if (isToggle()) {
                light.toggle();
            } else if (isTurnOn()) {
                light.turnOn();
        ...
        boolean isTurnOn(){
            return value.startsWith("turn on");
        }
        boolean isTurnOff(){
            return value.startsWith("turn off");
        }
        boolean isToggle(){
            return value.startsWith("toggle");
        }

        Rectangle getRectangle(){
            return new Rectangle(getFirstCorner(), getOtherCorner());
        };

        Corner getFirstCorner(){
            var tokens = tokens();
            if (isToggle()) {
                return new Corner(Integer.parseInt(tokens[1]), Integer.parseInt(tokens[2]));
            }
            return new Corner(Integer.parseInt(tokens[2]), Integer.parseInt(tokens[3]));
        }

        Corner getOtherCorner(){
            var tokens = tokens();
            if (isToggle()) {
                return new Corner(Integer.parseInt(tokens[4]), Integer.parseInt(tokens[5]));
            }
            return new Corner(Integer.parseInt(tokens[5]), Integer.parseInt(tokens[6]));
        }
    ...
    record Rectangle(Corner corner, Corner otherCorner){}
