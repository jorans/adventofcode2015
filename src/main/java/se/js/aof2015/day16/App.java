...
public class App {
    ...
    public static void part1(){
        System.out.println(Util.input(data).stream()
                .map(App::parseIntermediate)
                .map(App::assmbleSue)
                .filter(Sue::isSimilarPart1)
                .map(Sue::id)
                .toList());
    }
    ...
    public static void part2() {
        System.out.println(Util.input(data).stream()
                .map(App::parseIntermediate)
                .map(App::assmbleSue)
                .filter(Sue::isSimilarPart2)
                .map(Sue::id)
                .toList());
    }
    public static Sue assmbleSue(Map<String, OptionalInt> map) {
        return new Sue(
                map.get("Sue").getAsInt(),
                map.getOrDefault("akitas", OptionalInt.empty()),
                map.getOrDefault("cars", OptionalInt.empty()),
                map.getOrDefault("cats", OptionalInt.empty()),
                map.getOrDefault("children", OptionalInt.empty()),
                map.getOrDefault("goldfish", OptionalInt.empty()),
                map.getOrDefault("perfumes", OptionalInt.empty()),
                map.getOrDefault("pomeranians", OptionalInt.empty()),
                map.getOrDefault("samoyeds", OptionalInt.empty()),
                map.getOrDefault("trees", OptionalInt.empty()),
                map.getOrDefault("vizslas", OptionalInt.empty())
        );
    }

    static Map<String, OptionalInt> parseIntermediate(String input) {
        var parts = input.replaceAll("[:,]", "").split(" ");

        var paramValues = new HashMap<String, OptionalInt>();
        for (int i = 0; i < parts.length-1; i += 2) {
            paramValues.put(parts[i], OptionalInt.of(Integer.parseInt(parts[i + 1])));
        }

        return paramValues;
    }

    static Sue foo(String line){
    ...
    record Sue(
            ...
            OptionalInt cars,
            OptionalInt cats,
            OptionalInt children,
            OptionalInt goldfish,
            OptionalInt perfumes,
            OptionalInt pomeranians,
            OptionalInt samoyeds,
            OptionalInt trees,
            OptionalInt vizslas
    ){
        boolean isSimilarPart1() {
            return (this.akitas.isEmpty() || this.akitas.getAsInt() == 0) &&
            (this.cars.isEmpty() || this.cars.getAsInt() == 2) &&
            (this.cats.isEmpty() || this.cats.getAsInt() == 7 ) &&
            (this.children.isEmpty() || this.children.getAsInt() == 3) &&
            (this.goldfish.isEmpty() || this.goldfish.getAsInt() == 5) &&
            (this.perfumes.isEmpty() || this.perfumes.getAsInt() == 1) &&
            (this.pomeranians.isEmpty() || this.pomeranians.getAsInt() == 3) &&
            (this.samoyeds.isEmpty() || this.samoyeds.getAsInt() == 2) &&
            (this.trees.isEmpty() || this.trees.getAsInt() == 3) &&
            (this.vizslas.isEmpty() || this.vizslas.getAsInt() == 0);
        }
        boolean isSimilarPart2() {
            return (this.akitas.isEmpty() || this.akitas.getAsInt() == 0) &&
            (this.cars.isEmpty() || this.cars.getAsInt() == 2) &&
            (this.cats.isEmpty() || this.cats.getAsInt() > 7 ) &&
            (this.children.isEmpty() || this.children.getAsInt() == 3) &&
...
/*
[Compounds[id=40, akitas=null, cars=null, cats=null, children=null, goldfish=1, perfumes=null, pomeranians=0, samoyeds=null, trees=null, vizslas=null],
 Compounds[id=288, akitas=null, cars=null, cats=null, children=null, goldfish=0, perfumes=null, pomeranians=1, samoyeds=null, trees=null, vizslas=null],
 Compounds[id=323, akitas=null, cars=null, cats=null, children=null, goldfish=0, perfumes=1, pomeranians=null, samoyeds=null, trees=null, vizslas=null],
 Compounds[id=411, akitas=null, cars=null, cats=null, children=null, goldfish=2, perfumes=null, pomeranians=0, samoyeds=null, trees=null, vizslas=null]]
